...
# Create a queue for request on user details after a bid is closed
# (e.g. auth.request)

# Wildcard allows more types of requests to be sent if required in the future.

queue_name_consume = "User_Details_Request"
channel.queue_declare(queue=queue_name_consume, durable=True)
channel.queue_bind(
    exchange=exchange_name, queue=queue_name_consume, routing_key="auth.*"
)

# Create a shared queue for status related notifications that
# will be sent to process engine
# (e.g. status.auth.success, status.auth.fail)
