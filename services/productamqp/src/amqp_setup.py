...
# captures ALL product msgs (e.g. product.request)

queue_name_consume = "Product_Request"
channel.queue_declare(queue=queue_name_consume, durable=True)
channel.queue_bind(
    exchange=exchange_name, queue=queue_name_consume, routing_key="product.*"
)

# Create a shared queue for status related notifications that
# will be sent to process engine
# (e.g. status.product.success, status.product.fail)
