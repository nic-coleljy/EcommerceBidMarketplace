...
# captures ALL payment msgs (e.g. payment.new, payment.cancel)

queue_name_consume = "Payment_Invoice_Request"
channel.queue_declare(queue=queue_name_consume, durable=True)
channel.queue_bind(
    exchange=exchange_name, queue=queue_name_consume, routing_key="payment.*"
)

# Create a shared queue for status related notifications that
# will be sent to process engine
# (e.g. status.payment.success, status.payment.fail)
